...
Correcte aanpak
---------------
Gebruik ?format=car op de gateway. Dit vraagt de gateway om de volledige
DAG voor die CID te exporteren als een geldig CAR-bestand, inclusief:
  - alle onderliggende chunk-blokken
...
Optioneel:
  --gateway  Andere IPFS gateway (standaard: https://ipfs.io)
...
def fetch_car_for_cid(cid: str, gateway: str, timeout: int, retries: int) -> bytes:
    """
    Fetch the complete DAG-CAR for a single CID from the gateway.

    De URL ?format=car laat de gateway de volledige UnixFS DAG exporteren
    als CARv1, inclusief alle chunk-blokken voor grote bestanden.
    """
...
def main():
    ...
    parser.add_argument("--gateway", default="https://ipfs.io", help="IPFS gateway URL")
